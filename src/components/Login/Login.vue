<template>
	<div class="login">
		<div class="loginnow">
			...
			<p>
				<!-- <router-link :to="/reg/+type">账号注册 </router-link> -->
				<a href="#/reg/upd"> 账号注册</a>
				<a href="javascript:;" v-if="upd" @click="upwdLogin"> 验证码登录</a>
				<a href="javascript:;" v-else @click="upwdLogin"> 密码登录</a>
				<!-- <router-link v-if="upd":to="/login/pwd" @click="upwdLogin"> 密码登录</router-link>
				<router-link v-else :to="/login/code" @click="upwdLogin"> 验证码登录</router-link> -->
			</p>
		...
		methods: {
			...
			upwdLogin(){
				...
				if(this.$route.params.type=='pwd'){
					this.$router.replace('/login/code')
					this.$router.go(0)
				}else{
					this.$router.replace('/login/pwd')
					this.$router.go(0)
				...
				if(this.num>5){
					...
					if(this.upd){ // 账号密码登录
						if(this.uname ==null){ // 用户名为空
							...
							this.$store.commit('getuname',localStorage.getItem("name")) // main.js中的方法
						}else{ // 账号或密码错误
							this.error = 1
							this.upwd=null
							this.num++ // 密码/验证码错误次数累加
						}
					}else{ //验证码登录
