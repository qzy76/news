		...
		<div class="loginnow">
			...
			<form>
				<div v-if="upd === 'pwd'">
					...
					<p class="mb5"><input type="password" autocomplete="off" placeholder="请再次输入密码" v-model="rupwd" @keyup="change"></p>
				</div>
				<div v-else-if="upd === 'code'">
				...
				<div v-else>
					<p style="margin-top: -18px; margin-bottom: -18px;">请添加您的基本信息</p>
					<p><input type="text" placeholder="请输入用户名" v-model="uname" @keyup="change"></p>
					<p class="mb5"><input type="password" autocomplete="off" placeholder="请输入密码" v-model="upwd" @keyup="change"></p>
					<p class="mb5"><input type="password" autocomplete="off" placeholder="请再次输入密码" v-model="rupwd" @keyup="change"></p>
				</div>
			...
			<p class="m0">
				...
				<span v-if="error ==10">如无法正常注册，请联系管理员</span>
				<span v-if="error ==11">两次密码不一致</span>
				<span v-if="error ==12">当前手机号已被注册</span>
			</p>
			<p class="mt5"><input type="button" @click="reg" value="进入头条"></p>
			<p>点击查看"<a href="javascript:;">用户协议</a>"和"<a href="javascript:;">隐私政策</a>"</p>
			<div>
				<p>
					<a href="#/login/pwd">密码登录 </a> | 
					<a v-if="upd" href="#/reg/code" @click="upwdReg"> 验证码注册</a>
					<a v-else href="#/reg/pwd" @click="upwdReg"> 密码注册</a>
				</p>
			</div>
	...
	export default {
		...
		mounted(){
			if(this.$route.params.type=='code'){
				this.upd='code'
			}else if(this.$route.params.type=='addnews'){
				this.upd='addnews'
			}else{
				this.upd='pwd'
				this.$router.replace('/reg/pwd')
			}
		},
		methods: {
			ComeBreak(){
			...
			upwdReg(){
				this.error!=10?0:10
				if(this.$route.params.type=='pwd'){
					this.$router.replace('/reg/code')
					this.$router.go(0)
				}else if(this.$route.params.type=='addnews'){
					this.$router.replace('/reg/addnews')
					this.$router.go(0)
				}else{
					this.$router.replace('/reg/pwd')
					this.$router.go(0)
			...
			sendCode(){
				if(this.codenum>=3){
					...
					this.error = 6
					return false
				}else if(!(/^1[3456789]\d{9}$/.test(this.uphone))) { // 手机号不存在
					this.error = 8
					return false
				}else if(localStorage.getItem("uphone") ==this.uphone){ // 手机号已经被注册
					this.error = 12
					return false
				}else{
				...
				if(this.codenum>2){
					...
					if(this.upd=='upd'){ // 账号密码登录
						if(this.uname ==null){ // 用户名为空
							...
							this.error = 4
							return false
						}else if(this.upwd !=this.rupwd){ // 密码不一致
							this.error = 11
							return false
						...
						if(this.uphone==null){ // 手机号为空
							...
							this.error = 12
							return false
						}else if(!(/^1[3456789]\d{9}$/.test(this.uphone))){ // 手机号错误
							this.error = 8
							return false
						}else if(this.checkCode==this.code){ // 验证成功
							this.$router.replace('/reg/addnews')
							this.$router.go(0)
							this.error = 0
						}else{
					...
					}else if(this.upd=='addnews'){
						localStorage.setItem('name',this.uname)
						let upwd = this.$md5(this.upwd)
