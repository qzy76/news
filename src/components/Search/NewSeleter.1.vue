		...
		<div v-else class="newTitle">
			...
			<div ref="newtitle" v-show="!news" class="allnews" v-for="(item,index) in details" :key="index">
				<div class="break">
					...
					<select v-model="form" @change="lang1">
...
// 					{lang:'vie',name:'越南语'}
				...
				html:null ,// 新闻内容
				appid:'20171122000098516',
				keycode:'LlgO2A_6EezVBcQkLmia'
...
// 				keycode:'9s_UH6qxJ042fJIRV_my'
		...
		created(){
			this.form = this.lang[0].lang; // 设置默认选对个，不设置的话不显示
		},
		methods: {
			look(id) {
				let that = this;
			...
			lang1(){ // 百度翻译
				...
				let str1 = this.appid+this.html+salt + this.keycode;
				let sign = this.$md5(str1);
				console.log('http://api.fanyi.baidu.com/api/trans/vip/translate?q='+this.html+'&appid='+this.appid+'&from='+this.oldlang+'&to='+this.form+'&salt='+salt+'&sign='+sign);
				this.$jsonp('http://api.fanyi.baidu.com/api/trans/vip/translate',{
					...
					console.log(data);
					// that.html = data.trans_result[0].dst;
			     }).catch(function(err){
			         console.log(err);
			 });
			},
